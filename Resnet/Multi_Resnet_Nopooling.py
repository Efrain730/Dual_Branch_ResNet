...
model_name='Multi_Resnet_Nopooling'
...
def block(input, in_channel, out_channel1, out_channel2, conv_ksize, Block_name, image_trans=False, dimention_trans=False):
    ...
    result_summary = tf.nn.dropout(result_summary,keep_prob=keep_prob)
...
#32*32
res_normal_input = single_layer(input=x_normal_image,in_channel=1,out_channel=64,conv_ksize=2,layer_name='res_normal_input',relu=True,trans=True)
res_lamda_input = single_layer(input=x_lamda_image,in_channel=3,out_channel=64,conv_ksize=2,layer_name='res_lamda_input',relu=True,trans=True)

#16*16*64
section1_normal = section(input=res_normal_input,
                       in_channel=res_normal_input.get_shape()[3].value,
                       out_channel1=64,
                       out_channel2=64,
                       conv_ksize=2,
...
section1_lamda = section(input=res_lamda_input,
                       in_channel=res_lamda_input.get_shape()[3].value,
                       out_channel1=64,
                       out_channel2=64,
                       conv_ksize=2,
...
#2*2*512
sum_avg = tf.concat([section4_normal,section4_lamda],axis=3)

size_sum_avg = sum_avg.get_shape()[1].value ** 2
channel_sum_avg = sum_avg.get_shape()[3].value

len_sum_avg = int(size_sum_avg * channel_sum_avg)

with tf.variable_scope('Full_Connected_Layer1'):
    weight = tf.get_variable('weight',[len_sum_avg,2048],initializer=tf.contrib.layers.xavier_initializer(uniform=initializer_uniform))
    tf.add_to_collection('losses', tf.contrib.layers.l2_regularizer(L2_parameter)(weight))
    bias = tf.get_variable('bias', [2048], initializer=tf.constant_initializer(constant_initializer_parameter))
    sum_avg_flat = tf.reshape(sum_avg,[-1,len_sum_avg])
